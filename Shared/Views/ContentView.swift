                ...
                HStack(alignment: .top, spacing: 16) {
                    ...
                    GroupBox(L10n.dashboardPortfolioTrend) {
                        ...
                        if history.isEmpty {
                            ...
                            Group {
                                #if os(macOS)
                                PortfolioTrendChart(history: history, sp500History: sp500History.isEmpty ? nil : sp500History)
                                #else
...
private enum PortfolioChartSeries: String, CaseIterable, Plottable {
    case portfolio
    case sp500
}
...
struct PortfolioTrendChart: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: compact ? 4 : 8) {
            ...
            if (valueChange != nil) || (sp500History != nil && !(sp500History?.isEmpty ?? true)) {
                HStack {
...
struct PortfolioTrendChartShared: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: compact ? 4 : 8) {
            // Legend with date range and change percentage
            if let start = startDate, let end = endDate {
                HStack {
                    if compact {
                        ...
                        Text("\(L10n.summaryFrom) \(dateFormatter.string(from: start)) - \(dateFormatter.string(from: end))")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                    ...
                    if !compact {
                        Text(L10n.chartDataPoints(history.count))
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
